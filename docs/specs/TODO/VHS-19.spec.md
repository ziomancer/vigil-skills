...
> **Correction carried from round-1 review (load-bearing).** The brief, `docs/compound-engineering-evaluation.md`, and the v1 of this spec all named **"OpenClaw"** as an inherited CE smoke-test target. Verified against the upstream tree (`src/targets/` = `codex.ts, gemini.ts, kiro.ts, opencode.ts, pi.ts`): **OpenClaw is NOT a CE converter target.** "OpenClaw" is the fleet's *own* harness (served by the MCP server) — unrelated to what CE emits. The real adapter is **OpenCode** (`opencode.ts`). This spec uses **OpenCode** (Codex fallback) throughout; the brief's OpenClaw mentions were corrected in round 1, and its separate **"Claude Code passthrough"** framing — also a category error (Claude Code is the converter's *input*, not a `--to` target) — was corrected alongside (brief Scope #4 / Done-when #3); see D6 path (a). `docs/compound-engineering-evaluation.md`'s converter-target mislabel is corrected to **OpenCode** in this PR (lines 22 and 133); line 107's "OpenClaw/Calvin" is intentionally retained — there the term correctly names the fleet's *own* harnesses served by the MCP server, not a CE converter target.
