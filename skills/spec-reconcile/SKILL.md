...
4. Read `~/.claude/skills/ship-spec/states.json` (`~/.claude/` on Unix; `%USERPROFILE%\.claude\` on Windows). Look up ticket prefix to get `project_id`, `namespace`, and `project_identifier`. If prefix not found, halt with `Project prefix "<project_prefix>" not found in states.json. Add the project before running spec-reconcile.` — without `project_id`, the Plane state check in step 5 cannot proceed.
